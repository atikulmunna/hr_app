...
// An employee's pay structure in their entity's currency (T-2.1).
...
export class CompensationService {
  ...
  forEmployee(employeeId: string): Promise<CompensationSummary> {
    return this.db.withTenant(async (m) => {
      const employee = await this.findEmployee(m, employeeId);
      const currencyCode = await this.currencyFor(m, employee);
      const lines = await this.lines(m, employeeId);
      return { employeeId, currencyCode, ...totals(lines) };
    });
  }

  // Sets (or replaces) the amount for one component. The currency is the
  // employee's entity currency; an explicit mismatching currency is rejected
  // rather than silently converted (FR-M4-03).
  async set(
    employeeId: string,
    ...
    await this.db.withTenant(async (m) => {
      ...
      await this.assertSingleBasic(m, employeeId, component);
      ...
      await this.audit.record(
        {
          ...
          after: { code: component.code, amount, currencyCode },
        },
        m,
      );
    });
  ...
  // 'basic' is the anchor of a structure, so an employee has at most one.
  private async assertSingleBasic(
    ...
    const rows = await this.lines(m, employeeId);
  ...
  private async lines(
    ...
    const rows = await m.query(
      `SELECT epc.id,
        ...
        ORDER BY CASE pc.component_type
                   WHEN 'basic' THEN 0
